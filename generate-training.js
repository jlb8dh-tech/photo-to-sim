...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Start async orchestration (don't await)
...
async function orchestrateTraining(jobId, photoBase64) {
  try {
        ...
        messages: [
          {
            role: 'user',
            content: [
              { type: 'text', text: 'Analyze this facility photo for safety hazards and create a training scenario script. Return as JSON: {scenario, hazards, risks}' },
    ...
    const questions = [
      { question: 'What was the main hazard identified?', options: scriptJson.hazards },
      { question: 'How would you mitigate this risk?', options: ['Report to supervisor', 'Ignore it', 'Wait for inspection'] }
    ];
